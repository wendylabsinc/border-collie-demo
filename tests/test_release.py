...
def test_v31_rollback_release_identity_matches_root_media_and_stagefiles() -> None:
    root = Path(__file__).resolve().parents[1]
    descriptor = json.loads((root / "wendy.json").read_text(encoding="utf-8"))
    release_id = "stage-camera-v31-frame-path-rollback"
    build_label = (
        "stage-camera-v31-frame-path-rollback "
        "(codex/stage-camera-v31-frame-path-rollback)"
    )

    assert descriptor["version"] == "1.0.34-stage-camera"
    ...
    for environment in (app_env, media_env):
        ...
        assert environment["BORDER_COLLIE_CONFIG_SCHEMA"] == "14"
    ...
    for stagefile in (root_stagefile, media_stagefile):
        ...
        assert 'BORDER_COLLIE_CONFIG_SCHEMA: "14"' in stagefile
