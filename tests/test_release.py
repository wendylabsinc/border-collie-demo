...
def test_v25_release_identity_matches_root_media_and_stagefiles() -> None:
    root = Path(__file__).resolve().parents[1]
    descriptor = json.loads((root / "wendy.json").read_text(encoding="utf-8"))
    release_id = "stage-camera-v25-apple-pear-throughput-v1"
    build_label = (
        "stage-camera-v25-apple-pear-throughput-v1 "
        "(codex/stage-camera-v21-search-policy-abc)"
    )

    assert descriptor["version"] == "1.0.28-stage-camera"
    ...
    for environment in (app_env, media_env):
        ...
        assert environment["BORDER_COLLIE_CONFIG_SCHEMA"] == "9"
    ...
    for stagefile in (root_stagefile, media_stagefile):
        ...
        assert 'BORDER_COLLIE_CONFIG_SCHEMA: "9"' in stagefile
