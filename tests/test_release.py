...
def test_v20_release_identity_matches_root_media_and_stagefiles() -> None:
    root = Path(__file__).resolve().parents[1]
    descriptor = json.loads((root / "wendy.json").read_text(encoding="utf-8"))
    release_id = "stage-camera-v20-continuous-track"
    build_label = (
        "stage-camera-v20-continuous-track "
        "(codex/stage-camera-v20-continuous-track)"
    )

    assert descriptor["version"] == "1.0.23-stage-camera"
