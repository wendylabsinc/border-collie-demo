...
| `codex/apple-confidence-50` | Derives from deployed v26 and changes only the red-apple experiment: app acquisition, media stability, continued tracking, and scorecard policy use one 0.50 confidence floor. Release `stage-camera-v27-apple-confidence-50`, schema 11, app version `1.0.30-stage-camera`. | The equal floor avoids an inverted 0.50-acquisition/0.55-tracking band that could accept a lock and immediately reject the same-quality track. Pear and banana policies are regression-pinned unchanged. This candidate is software-only until a supervised deployment and fruit run qualify it. |
