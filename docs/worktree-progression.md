...
| `codex/stage-camera-v29-persistent-fruit-track` | Replaces the search-to-approach identity token with one mission-lifetime fruit track. Full-frame inference is authoritative on every processed frame; an agreeing crop may refine but cannot create or erase identity. Release `stage-camera-v29-persistent-fruit-track`, schema 13, app version `1.0.32-stage-camera`. | Unit/integration tests prove the state model, same-object mission seam, fail-closed authority, paired frame/command trace, and replay API. The deployed supervised Apple run `3ed9a3bb-116c-409f-81f2-149508e8a648` retained one acquisition epoch across all phases, but failed after two low-confidence frames only 0.127 s apart even though full-frame/crop still showed a centered Apple and filtered confidence remained 0.6291. It also spent 85 approach commands aligning from an extreme `x=0.0438` search lock and sent no forward pulse. V29 is physically characterized, not qualified. |
