...
| `codex/stage-camera-v21-search-policy-abc` | Integrates the three search experiments behind one immutable per-run selector: three-frame `fast-lock`, `0.50 rad/s` `slow-sweep`, and bounded `double-back`. Current release `stage-camera-v22-black-box`, schema 6, app version `1.0.25-stage-camera`, adds per-run black-box traces without changing the randomized A/B/C policy selector. | Derived from the five-attempt v19 characterization: two apple searches produced high-confidence proposals but only four and one consecutive frames at `1.0 rad/s`. The three source experiments passed deterministic replays independently; the combined release must still complete its matched 10+10+10 physical trial. |
| `codex/perception-throughput-v1` | Adds a reversible perception scheduler experiment: `baseline` preserves v22 serialization while `throughput-v1` publishes motion evidence before latest-only odometry/preview lanes and staggers missing apple/banana full-frame and search-crop passes across advancing frames. A crop candidate keeps that normalized route until loss so interleaved full-frame misses cannot reset acquisition. Release `stage-camera-v23-perception-throughput`, schema 7, app version `1.0.26-stage-camera`. | Derived from v22 read-only evidence of 14.285 source FPS but 5.665 completed detector FPS. Software tests and synthetic scheduling evidence are required before deployment; no physical performance or reliability improvement is claimed. The banana specialist remains `.pt` until a Jetson-exported TensorRT engine passes matched output and latency qualification. |
