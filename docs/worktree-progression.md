...
| `codex/stage-camera-v21-search-policy-abc` | Integrates the three search experiments behind one immutable per-run selector: three-frame `fast-lock`, `0.50 rad/s` `slow-sweep`, and bounded `double-back`. Current release `stage-camera-v23-apple-pear-confidence`, schema 7, app version `1.0.26-stage-camera`, retains v22 black-box traces and gives apple the same 0.65 acquisition and 0.55 tracking floors as pear. | Derived from the five-attempt v19 characterization and v22 randomized trace. The threshold change is software-tested but requires a new supervised Apple/Pear physical result before qualification. |
