...
| `codex/stage-camera-v21-search-policy-abc` + throughput integration | Combines the v23 Apple/Pear confidence policy with the reversible scheduler. Baseline cohort `stage-camera-v24-apple-pear-pipeline-baseline` used schema 8/app `1.0.27`; comparison cohort `stage-camera-v25-apple-pear-throughput-v1` uses schema 9/app `1.0.28`. | Both retain Apple/Pear acquisition at 0.65 and close tracking at 0.55. The matched Apple/Pear/Banana cohorts each scored 0/3. Throughput-v1 improved processed cadence from 7.00 to 11.56 FPS and reduced the cumulative drop ratio from 50.5% to 10.9%, but Apple search lock, close Pear off-axis loss, and post-Home heading restoration still failed. |
