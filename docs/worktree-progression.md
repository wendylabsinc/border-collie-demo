...
| `codex/stage-camera-v21-search-policy-abc` v27 integration | Derives from deployed v26 and combines three reviewable changes: the Apple policy uses one 0.50 acquisition/tracking floor, approach translation is confined to the middle 40% of the frame with a bounded 0.20 rad/s recenter, and the soak harness waits for authoritative automatic recovery plus a 0.50 m inter-run stage margin. Release `stage-camera-v27-center-corridor-apple50`, schema 11, app version `1.0.30-stage-camera`. | The supervised randomized cohort completed 1/5: one Apple completed at 0.0867 m from Home; the second Apple timed out in Arrival; Pear failed once off-axis and once in search; Banana failed off-axis. All four recovery paths completed inside 0.083 m. The middle-40% recenter physically paused, turned at 0.20 rad/s, reacquired, and resumed, but the independent tighter close-handoff corridor still cancelled Pear/Banana. V27 is physically characterized, not repeatability-qualified. |
| `codex/stage-camera-v21-search-policy-abc` v28 evidence integration | Preserves v27 motion policy and adds one bounded annotated camera frame while Woof is down, for both the audience action and eligible failed-run recovery. Release `stage-camera-v28-lie-down-evidence`, schema 12, app version `1.0.31-stage-camera`. | Motivated by the operator labeling app-failed run 1 physically successful, app-success run 4 far from its apple, runs 2/3 unknown, and run 5 far from its apple. Success remains compact but retains its lie-down JPEG; failures retain the full archive plus recovery lie-down JPEG. `scripts/fruit-soak check` provides a read-only release/readiness/disarm/camera proof, and `scripts/fruit-soak test` auto-selects Python 3.14. Prepared and software-tested while Woof was offline; not deployed or physically tested. |
| `codex/stage-camera-v31-frame-path-rollback` | Restores the exact v28 perception and qualified-tracker implementation at commit `16f3586`, before the v29 persistent mission track and full-frame-first rewrite. Release `stage-camera-v31-frame-path-rollback`, schema 14, app version `1.0.34-stage-camera`. | Created after v29/v30 repeatedly failed with `persistent pear track was lost: confirmed_full_frame_loss`. This is an explicit rollback experiment: it retains v28's throughput-v1 scheduled full-frame/crop behavior and phase-local qualified tracking, while using a new release identity so results cannot be confused with the historical v28 cohort. |
| `codex/stage-camera-v31-frame-path-rollback` v32 confidence integration | Keeps the restored v28 frame path and replaces the established track's raw per-frame confidence gate with a configurable arithmetic mean over fresh advancing frames. Release `stage-camera-v32-confidence-average`, schema 15, app version `1.0.35-stage-camera`. | Derived from v31 Pear run `e94567c0-3c32-4d44-abb2-8f442459c430`: raw terminal confidence was 0.5254 versus the 0.55 tracking floor, while the exact ten-frame mean was 0.6002. The replay remains in close-range steering. This candidate is software-tested but not yet deployed or physically qualified. |
| `codex/stage-camera-v31-frame-path-rollback` v33 base-motion integration | Keeps v32 confidence averaging and current fail-closed perception gates, but restores the base demo's proven sustained `1.0 m/s` factory-avoidance approach through both normal and close-range tracking. The one bounded final push remains `0.6 m/s` for one second. Release `stage-camera-v33-base-motion-approach`, schema 16, app version `1.0.36-stage-camera`. | Derived from v32 Pear run `89e1803e-c508-4762-b5c2-009e879c76d8`: eleven `0.55 m/s` heartbeats spanned 1.22 seconds and measured about 0.35 m before perception removed authority. This candidate restores base-like translation without rolling back stale/wrong-identity/generation/camera/off-axis stops. Physical behavior remains unqualified until a supervised run. |
