...
| `codex/stage-camera-v21-search-policy-abc` + throughput integration | Combines the v23 Apple/Pear confidence policy with the reversible scheduler. The first comparison cohort is release `stage-camera-v24-apple-pear-pipeline-baseline`, schema 8, app version `1.0.27-stage-camera`, with `PERCEPTION_PIPELINE_PROFILE=baseline`. | The combined code preserves Apple/Pear acquisition at 0.65 and close tracking at 0.55. A second distinctly stamped `throughput-v1` cohort will follow the baseline three-fruit run so physical results are attributable to the scheduler profile. |
