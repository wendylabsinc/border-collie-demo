...
| --- | --- | ---: | ---: | --- |
| `BORDER_COLLIE_SEARCH_MOTION_PATH` | enum | `sport_yaw` | `sport_yaw` or `factory_avoidance` | Search is yaw-only. `sport_yaw` avoids the current serial factory-avoidance verification latency; this exact path/rate combination is software-tested and still needs one physical qualification. Set `factory_avoidance` to roll back without rebuilding; neither path authorizes translation during search. |
| `BORDER_COLLIE_GUIDANCE_SEARCH_YAW_RPS` | rad/s | `0.40` | `0.40..0.80` | `0.40` is the physically successful 5/5 scan rate. Search remains bounded to one measured revolution. |
| `BORDER_COLLIE_GUIDANCE_FOCUS_YAW_RPS` | rad/s | `0.40` | `0.40..0.80`, no greater than search yaw | A qualified off-center fruit changes the yaw-only search into fine alignment. This value never authorizes translation. |
...
| `BORDER_COLLIE_GUIDANCE_FOCUS_MINIMUM_YAW_RPS` | rad/s | `0.40` | `0.40..focus yaw` | The 5/5 demo proved this rate during search; the new Sport-yaw execution path still needs physical qualification. Crossing frame center resets the next correction to the full focus yaw in the opposite direction. |
...
| `BORDER_COLLIE_GUIDANCE_CENTER_TOLERANCE_RATIO` | frame-width ratio from center | `0.05` | greater than `0`, less than outer corridor | Three fresh samples must be inside this band before lock. Set `0.08` together with progressive focus disabled to restore the prior lock profile. |
