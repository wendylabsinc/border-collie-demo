...
class GuidanceConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.40 <= self.focus_yaw_rps <= self.search_yaw_rps:
            raise ValueError(
                "focus_yaw_rps must respect the proven 0.40 rad/s search floor "
        ...
        if not 0.40 <= self.focus_minimum_yaw_rps <= self.focus_yaw_rps:
            raise ValueError(
                "focus_minimum_yaw_rps must respect the proven 0.40 rad/s "
    ...
    def from_env(cls) -> GuidanceConfig:
        ...
        config = cls(
            ...
            focus_minimum_yaw_rps=float(
                os.environ.get(prefix + "FOCUS_MINIMUM_YAW_RPS", "0.40")
            ),
