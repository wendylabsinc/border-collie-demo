...
ARCHITECTURE = json.loads(
    Path(__file__).with_name("architecture.json").read_text(encoding="utf-8")
)
ARCHITECTURE_VERSION = int(ARCHITECTURE["version"])
BLOCKS = tuple(tuple(int(value) for value in block) for block in ARCHITECTURE["blocks"])
...
def _build_graph(resolution: int) -> tuple[Any, dict[str, np.ndarray], dict[str, Any]]:
    ...
    with Graph(
        ...
        pyramid_channels = int(ARCHITECTURE["feature_pyramid_channels"])
        pyramid: dict[int, tuple[Any, int, int]] = {}
        previous: tuple[Any, int, int] | None = None
        for stride in reversed(tuple(int(value) for value in ARCHITECTURE["head_strides"])):
            ...
            lateral, _ = conv(
                ...
                1,
                1,
                f"laterals.stride_{stride}",
                False,
                feature_spatial,
            )
            if previous is not None:
                previous_value, previous_channels, previous_spatial = previous
                if previous_channels != pyramid_channels or feature_spatial != previous_spatial * 2:
                    raise RuntimeError("feature pyramid shape contract changed")
                expanded = ops.reshape(
                    previous_value,
                    [1, previous_spatial, 1, previous_spatial, 1, pyramid_channels],
                )
                repeated = ops.broadcast_to(
                    expanded,
                    [1, previous_spatial, 2, previous_spatial, 2, pyramid_channels],
                )
                previous_value = ops.reshape(
                    repeated,
                    [1, feature_spatial, feature_spatial, pyramid_channels],
                )
                lateral = ops.add(lateral, previous_value)
            lateral = ops.relu(lateral)
            refined, _ = conv(
                lateral,
                pyramid_channels,
                pyramid_channels,
                ...
                1,
                pyramid_channels,
                f"refinements.stride_{stride}.depthwise",
                True,
                feature_spatial,
            )
            refined, _ = conv(
                ...
                1,
                1,
                f"refinements.stride_{stride}.pointwise",
                True,
                feature_spatial,
            )
            pyramid[stride] = (refined, pyramid_channels, feature_spatial)
            previous = pyramid[stride]

        outputs = []
        output_shapes = []
        for stride in tuple(int(value) for value in ARCHITECTURE["head_strides"]):
            feature, channels, feature_spatial = pyramid[stride]
            head, _ = conv(
                feature,
                channels,
                channels,
                ...
                1,
                channels,
                f"heads.stride_{stride}.depthwise",
                True,
                feature_spatial,
            )
            head, _ = conv(
                ...
                1,
                1,
                f"heads.stride_{stride}.pointwise",
                True,
                feature_spatial,
            )
            head, _ = conv(
                head,
                channels,
                int(ARCHITECTURE["head_channels"]),
                1,
                1,
                1,
                f"heads.stride_{stride}.output",
                False,
                feature_spatial,
            )
            outputs.append(head)
            output_shapes.append(
                [1, feature_spatial, feature_spatial, int(ARCHITECTURE["head_channels"])]
            )
